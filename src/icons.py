...
fácil de colorir no CSS) e uma função `make_href(value, ctx)` que decide
se o item vira link clicável e qual URL usar (None = não é link, só
ícone + texto). `ctx` é o contexto completo do currículo (dá acesso a
outros campos, como full_name, para compor o link).
...
def _no_link(value: str, ctx: dict):
...
def _github_href(value: str, ctx: dict):
...
# `make_href`: função (value, ctx) -> URL ou None (None = não vira link)
CONTACT_TYPES = [
    {"key": "address", "icon": _ICON_ADDRESS, "make_href": _no_link},
    {"key": "phone", "icon": _ICON_PHONE, "make_href": _no_link},
    {"key": "email", "icon": _ICON_EMAIL, "make_href": _email_href},
